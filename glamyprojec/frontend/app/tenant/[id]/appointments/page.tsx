...
export default function AppointmentsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Funciones auxiliares para buscar nombres por ID o leer del JSON anidado
  const getCustomerName = (app: Appointment) => app.customer?.name || (app.customerId ? customers.find(c => c.id === app.customerId)?.name : '') || 'Desconocido';
  const getServiceName = (app: Appointment) => app.service?.name || (app.serviceId ? services.find(s => s.id === app.serviceId)?.name : '') || 'Desconocido';
  const getStaffName = (app: Appointment) => app.staff?.name || (app.staffId ? staff.find(s => s.id === app.staffId)?.name : '') || 'Desconocido';
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {appointments.map(app => (
                      <tr key={app.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-4">
                          <div className="font-medium text-slate-800">{getCustomerName(app)}</div>
                        </td>
                        <td className="p-4">
                          <div className="text-sm font-bold text-slate-700">{getServiceName(app)}</div>
                          <div className="text-xs text-slate-500">Atendido por: {getStaffName(app)}</div>
                        </td>
                        <td className="p-4">
